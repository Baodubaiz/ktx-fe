...
export default function ImportAssignmentModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await phanCongVeSinhService.importExcel(formData)
      ...
      message.error('Lỗi: ' + getApiError(error))
    } finally {
      setUploading(false)
    }
  }
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      const data = await phanCongVeSinhService.downloadTemplate()
      const blob = new Blob([data], {
        type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
      })
      saveAs(blob, 'phan_cong_ve_sinh_template.xlsx')
    } catch {
  ...
  return (
    <Modal
      title="Nhập dữ liệu phân công vệ sinh từ Excel"
      open={isOpen}
      onCancel={() => {
        onClose()
        setFileList([])
        ...
        <Alert
          ...
          description={
            <List
              size="small"
              dataSource={[
                'Tải file mẫu Excel và điền thông tin phân công theo đúng cấu trúc',
                'Các cột bắt buộc: Vị trí, Trung đội phụ trách',
                'Các cột tùy chọn: Thời gian',
