...
export default function UpdateAssignmentModal({ isOpen, onClose, assignment, onSuccess }: Props) {
  ...
  useEffect(() => {
    if (isOpen && assignment) {
      form.setFieldsValue({
        vi_tri_id: assignment.vi_tri_id,
        trung_doi_phu_trach: assignment.trung_doi_phu_trach,
        thoi_gian: assignment.thoi_gian ? dayjs(assignment.thoi_gian) : null,
      })
      fetchLookups()
    }
  }, [isOpen, assignment])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await phanCongVeSinhService.update(assignment.id, dto)
      ...
      message.error(getApiError(err, 'Cập nhật phân công thất bại'))
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <Modal
      title={`Chỉnh sửa phân công vệ sinh #${assignment?.id}`}
      open={isOpen}
      onCancel={() => {
        onClose()
        form.resetFields()
          ...
          <Select
            ...
            options={viTriList
              .reduce((acc: any[], v: any) => {
                ...
                if (!acc.find((o: any) => o.value === v.id)) {
