...
export default function ImportRoomModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const duplicateDetails: string[] = stats?.duplicateDetails ?? []
        ...
        Modal.warning({
          ...
          content: (
            <div style={{ marginTop: 10 }}>
              ...
              <p>Trùng mã phòng: <b style={{color: '#fa8c16'}}>{stats.duplicate ?? 0}</b></p>
              ...
              {duplicateDetails.length > 0 && (
                <div style={{ marginTop: 10, maxHeight: 160, overflow: 'auto', background: '#fff7e6', padding: 10, borderRadius: 4, border: '1px solid #ffd591' }}>
                  {duplicateDetails.map((msg: string, i: number) => (
                    <div key={i} style={{ fontSize: 12, color: '#d46b08', marginBottom: 4 }}>• {msg}</div>
                  ))}
                </div>
              )}
            </div>
      ...
      } else if ((stats?.duplicate ?? 0) > 0) {
        Modal.info({
          title: 'Import thành công, có bản ghi trùng',
          width: 600,
          content: (
            <div style={{ marginTop: 10 }}>
              <p>Tạo/Cập nhật thành công: <b style={{color: 'green'}}>{stats.success}</b></p>
              <p>Bản ghi trùng đã cập nhật: <b style={{color: '#fa8c16'}}>{stats.duplicate}</b></p>
              {duplicateDetails.length > 0 && (
                <div style={{ maxHeight: 200, overflow: 'auto', background: '#fff7e6', padding: 10, borderRadius: 4, border: '1px solid #ffd591' }}>
                  {duplicateDetails.map((msg: string, i: number) => (
                    <div key={i} style={{ fontSize: 12, color: '#d46b08', marginBottom: 4 }}>• {msg}</div>
                  ))}
                </div>
  ...
  return (
    <Modal
      title="Nhập dữ liệu phòng từ Excel"
      open={isOpen}
      onCancel={() => {
        onClose()
        setFileList([])
        ...
        <Alert
          ...
          description={
            <List
              size="small"
              dataSource={[
                'Tải file mẫu Excel và điền thông tin phòng theo đúng cấu trúc',
                'Các cột bắt buộc: Mã phòng, Tòa nhà, Tầng',
                'Các cột tùy chọn: Số giường, Loại phòng, Giới tính',
